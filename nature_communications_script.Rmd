...
Income_memory.m <- lm(TBX_ibam_scr ~ logIncome3 +AgeC_test + GenderEC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)]); summary(Income_memory.m)

Income_memory_nonlinear.m <- lm(TBX_ibam_scr ~ logIncome3 +AgeC + Age2 + Age3 + GenderEC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)]); summaryh(Income_memory_nonlinear.m)
#income significantly correlates with memory in both models
```

```{r vocabulary ~ income models}
Income_language.m <- lm(TBX_VOCAB_THETA ~ logIncome3 + GenderEC + AgeC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)]); summary(Income_language.m)

Income_language.m_nonlinear <- lm(TBX_VOCAB_THETA ~ logIncome3 + GenderEC + AgeC + Age2, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)]); summaryh(Income_language.m_nonlinear)
#vocab significantly correlates with memory in both models
```

```{r processing speed ~ income models (control analysis)}
processingspeed_income.m <- lm(TBX_pspac_scr~ logIncome3 + GenderEC + AgeC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & !is.na(TBX_ibam_scr)]); summary(processingspeed_income.m) 
...
memory_income_gap.m <- lm(TBX_ibam_scrZ ~ logIncome3 + GenderEC + AgeC_test, data = pingSubset);summary(memory_income_gap.m) 
...
language_income_gap.m <- lm(TBX_VOCAB_THETAZ ~ logIncome3 + GenderEC + AgeC_test, data = pingSubset); summary(language_income_gap.m)
...
```{r memory ~ income * age}
Income_memory_age.m <- lm(TBX_ibam_scr ~ AgeC * logIncome3 + GenderEC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)]); summaryh(Income_memory_age.m)
```

```{r vocab ~ income * age}
Income_language_age.m <- lm(TBX_VOCAB_THETA ~  AgeC * logIncome3 + GenderEC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)]); summaryh(Income_language_age.m)
```
...
pingSubset[!is.na(anterior_hpc) & !is.na(TBX_ibam_scr) & !is.na(posterior_hpc), TBX_ibam_scr_C := scale(TBX_ibam_scr)]
pingSubset[!is.na(anterior_hpc) & !is.na(TBX_ibam_scr) & !is.na(posterior_hpc), TBX_VOCAB_THETA_C := scale(TBX_VOCAB_THETA)]
pingSubset[!is.na(anterior_hpc) & !is.na(TBX_ibam_scr) & !is.na(posterior_hpc) & !is.na(TBX_pspac_scr), TBX_pspac_scr_C := scale(TBX_pspac_scr)]
```

```{r create long form data}
subset_long = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & !is.na(TBX_ibam_scr_C) & !is.na(TBX_pspac_scr_C), .(ID, logIncome3, TBX_ibam_scr_C, TBX_VOCAB_THETA_C, TBX_pspac_scr_C, anterior_hpc, DeviceSerialNumber, AgeC, GenderEC)] #subset variables of interest and convert to long form
subset_long = setDT(subset_long)
subset_long[, n_distinct(ID)]
subset_long = gather(subset_long, task, score, -c(ID, logIncome3, anterior_hpc, DeviceSerialNumber, AgeC, GenderEC))
...
Income_memory_income_subsample.m <- lm(TBX_ibam_scr ~ income3 * incomeSplit + AgeC + GenderEC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)]); summary(Income_memory_income_subsample.m)
##income (in raw dollars) has a larger effect on memory in lower income subsample
```

```{r vocab ~ income * incomesubsample}
Income_language_income_subsample.m <- lm(TBX_VOCAB_THETA ~ income3 * incomeSplit + AgeC + GenderEC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)]); summary(Income_language_income_subsample.m)
#income (in raw dollars) has a larger effect on vocab in lower income subsample
```

```{r processing speed ~ income * income subsample }
Income_processing_speed_income_subsample.m <- lm(TBX_pspac_scr ~ income3 * incomeSplit + AgeC+ GenderEC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)]); summary(Income_processing_speed_income_subsample.m)
...
Income_memory_income_subsample_low.m <- lm(TBX_ibam_scr ~ logIncome3 + AgeC + GenderEC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & incomeSplit == -1]); summary(Income_memory_income_subsample_low.m)

#low income (added nonlinear effect of age)
Income_memory_income_subsample_low.m_nonlinear <- lm(TBX_ibam_scr ~ logIncome3 + AgeC + Age2_test +GenderEC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & incomeSplit == -1]); summaryh(Income_memory_income_subsample_low.m_nonlinear)

#high income
Income_memory_income_subsample_high.m <- lm(TBX_ibam_scr ~ logIncome3 + AgeC + GenderEC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & incomeSplit == 1]); summaryh(Income_memory_income_subsample_high.m)
```

```{r vocabulary ~ income separately for low and high income subsamples}
#lower income group (linear effect of age only)_
Income_language_low_income.m <- lm(TBX_VOCAB_THETA ~ logIncome3 + GenderEC + AgeC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & incomeSplit == -1]); summary(Income_language_low_income.m)

#lower income gorup (added nonlinear effect of age)
Income_language_low_income.m_nonlinear <- lm(TBX_VOCAB_THETA ~ logIncome3 + GenderEC + AgeC + Age2, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & incomeSplit == -1]); summaryh(Income_language_low_income.m_nonlinear)

#higher income group
Income_language_high_income.m <- lm(TBX_VOCAB_THETA ~ logIncome3 + GenderEC + AgeC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & incomeSplit == 1]); summaryh(Income_language_high_income.m)
```
...
#full
Income_memory_minority_moderation.m <- lm(TBX_ibam_scr ~ logIncome3 * minority_status +AgeC+ GenderEC , data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)]); summaryh(Income_memory_minority_moderation.m)

#low income
Income_memory_minority_moderation.m_low_income <- lm(TBX_ibam_scr ~ logIncome3 * minority_status +AgeC+ GenderEC , data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & incomeSplit == -1]); summaryh(Income_memory_minority_moderation.m_low_income)

#control for minority status
#full sample
Income_memory_minority.m_full <- lm(TBX_ibam_scr ~ logIncome3 +AgeC + GenderEC + minority_status, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)]); summaryh(Income_memory_minority.m_full)

#lower income subsample
Income_memory_minority.m_lower_subsample <- lm(TBX_ibam_scr ~ logIncome3 +AgeC + GenderEC + minority_status, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & incomeSplit == -1]); summaryh(Income_memory_minority.m_lower_subsample)
```

```{r vocabulary scores ~ income + minority status; and * minority status}
#moderation with minority status
#full
Income_language_minority_moderation.m <- lm(TBX_VOCAB_THETA ~ logIncome3 * minority_status+ GenderEC + AgeC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)]); summaryh(Income_language_minority_moderation.m)

#low income
Income_language_minority_moderation.m_low_income_subsample <- lm(TBX_VOCAB_THETA ~ logIncome3 * minority_status+ GenderEC + AgeC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & incomeSplit == -1]); summaryh(Income_language_minority_moderation.m_low_income_subsample)


#control for minority status
#full
Income_language_minority.m_full <- lm(TBX_VOCAB_THETA ~ logIncome3 + GenderEC + AgeC +minority_status, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)]); summaryh(Income_language_minority.m_full)

#low income
Income_language_minority.m_low_subsample <- lm(TBX_VOCAB_THETA ~ logIncome3 + GenderEC + AgeC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & incomeSplit == -1]); summaryh(Income_language_minority.m_low_subsample)
```
...
#non minority status
Income_memory_nonminority.m <- lm(TBX_ibam_scr ~ logIncome3 +AgeC+ GenderEC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & incomeSplit == -1 & minority_status == -1]); summaryh(Income_memory_nonminority.m)

#minority status
Income_memory_minority_status.m <- lm(TBX_ibam_scr ~ logIncome3 +AgeC+ GenderEC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & incomeSplit == -1 & minority_status == 1]); summary(Income_memory_minority_status.m)
```

```{r vocabulary scores ~ income by minority status in lower income}
#non minority status
Income_language_nonminority_low.m <- lm(TBX_VOCAB_THETA ~ logIncome3 + GenderEC + AgeC , data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & incomeSplit == -1 & minority_status == -1]); summaryh(Income_language_nonminority_low.m)

#minority status
Income_language_minority_low.m <- lm(TBX_VOCAB_THETA ~ logIncome3 + GenderEC + AgeC , data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & incomeSplit == -1 & minority_status == 1]); summaryh(Income_language_minority_low.m)
```
...
Income_anterior.m <- lm(anterior_hpc ~ logIncome3 + AgeC + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(posterior_hpc)]); summary(Income_anterior.m)

pingSubset[, .(GenderEC, Gender)]
```

```{r posterior vols ~ income}
Income_posterior.m <- lm(posterior_hpc ~ logIncome3 + AgeC  + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(anterior_hpc)]); summary(Income_posterior.m)
```
...
Income_anterior_age.m <- lm(anterior_hpc ~ logIncome3 * AgeC + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(posterior_hpc)]); summary(Income_anterior_age.m)
```

```{r posterior vols ~ income * age}
Income_posterior_age.m <- lm(posterior_hpc ~ AgeC * logIncome3 + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(anterior_hpc)]); summary(Income_posterior_age.m)
```
...
Income_anterior_interaction_income.m <- lm(anterior_hpc ~ income3 * incomeSplit + AgeC + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(posterior_hpc)]); summary(Income_anterior_interaction_income.m) #results show that gains in income (in raw dollars) matter more for low than high subsample
```

```{r posterior vols ~ income * income subsample}
Income_posterior_interaction_income.m <- lm(posterior_hpc ~ income3 * incomeSplit+ AgeC + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(anterior_hpc)]); summary(Income_posterior_interaction_income.m)
```
...
#low income
Income_anterior_low_income.m <- lm(anterior_hpc ~ logIncome3 + AgeC + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(posterior_hpc) & incomeSplit == -1]); summary(Income_anterior_low_income.m)

#high income
Income_anterior_high_income.m <- lm(anterior_hpc ~ logIncome3 + AgeC + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(posterior_hpc) & incomeSplit == 1]); summaryh(Income_anterior_high_income.m)
```

```{r posterior ~ income separately for high and low income subsample}
# low income
Income_posterior_low_income.m <- lm(posterior_hpc ~ logIncome3 + AgeC + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(anterior_hpc) & incomeSplit == -1]); summary(Income_posterior_low_income.m)

#high income
Income_posterior_high_income.m <- lm(posterior_hpc ~ logIncome3 + AgeC + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(anterior_hpc) & incomeSplit == 1]); summary(Income_posterior_high_income.m)
...
#full sample 
Income_anterior_minority_status_interaction.m <- lm(anterior_hpc ~ logIncome3 * minority_status + AgeC + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(posterior_hpc)]); summary(Income_anterior_minority_status_interaction.m)

#lower income
Income_anterior_minority_status_interaction_low_income.m <- lm(anterior_hpc ~ logIncome3 * minority_status + AgeC + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(posterior_hpc) & incomeSplit == -1]); summary(Income_anterior_minority_status_interaction_low_income.m)

#lower income nminority status
Income_anterior_minority_status_low_income.m <- lm(anterior_hpc ~ logIncome3+ AgeC + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(posterior_hpc) & incomeSplit == -1 & minority_status == 1]); summary(Income_anterior_minority_status_low_income.m)

#lower income nonminority status
Income_anterior_nonminority_status_low_income.m <- lm(anterior_hpc ~ logIncome3+ AgeC + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(posterior_hpc) & incomeSplit == -1 & minority_status == -1]); summary(Income_anterior_nonminority_status_low_income.m)

#higher income
Income_anterior_minority_status_interaction_high_income.m <- lm(anterior_hpc ~ logIncome3 * minority_status + AgeC + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(posterior_hpc) & incomeSplit == 1]); summary(Income_anterior_minority_status_interaction_high_income.m)
```

```{r posterior hpc vols ~ income * minority status}
#full sample
Income_posterior_minority_status_interaction.m <- lm(posterior_hpc ~ logIncome3 * minority_status+ AgeC + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(anterior_hpc)]); summary(Income_posterior_minority_status_interaction.m)

#lower income only - posterior
Income_posterior_minority_status_interaction_low_income.m <- lm(posterior_hpc ~ logIncome3 * minority_status+ AgeC + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(anterior_hpc) & incomeSplit == -1]); summaryh(Income_posterior_minority_status_interaction_low_income.m)
```
...
```{r anterior hpc ~ income + minority status}
#full sample
Income_anterior_minority_status.m <- lm(anterior_hpc ~ logIncome3 + AgeC + GenderEC +  DeviceSerialNumber + minority_status, data = pingSubset[!is.na(posterior_hpc)]); summary(Income_anterior_minority_status.m)

#lower income only
Income_anterior_minority_status_low_income.m <- lm(anterior_hpc ~ logIncome3 + AgeC + GenderEC +  DeviceSerialNumber + minority_status, data = pingSubset[!is.na(posterior_hpc) & incomeSplit == -1]); summary(Income_anterior_minority_status_low_income.m)
```
...
longpingSubset_analysis <- setDT(gather(pingSubset[!is.na(anterior_hpc) & !is.na(posterior_hpc), 
                                             .(ID, anterior_hpc, posterior_hpc, logIncome3, AgeC, GenderEC, 
                                               DeviceSerialNumber2)], region, volume, -c(ID, AgeC, logIncome3, GenderEC, 
                                              DeviceSerialNumber2)))

longpingSubset_analysis[region == "posterior_hpc", region2 := 1]
longpingSubset_analysis[region == "anterior_hpc", region2 := 0]
...
interaction.model = lmer(volume ~  logIncome3 * region2 + GenderEC + AgeC + DeviceSerialNumber2  +(1|ID), data = longpingSubset_analysis); summaryh(interaction.model)
```
...
pingSubset[!is.na(anterior_hpc) & !is.na(posterior_hpc), anterior_hpc_plotting := residuals(lm(anterior_hpc ~ GenderEC  + AgeC + DeviceSerialNumber))]
pingSubset[!is.na(anterior_hpc) & !is.na(posterior_hpc), posterior_hpc_plotting := residuals(lm(posterior_hpc ~ GenderEC  + AgeC + DeviceSerialNumber))]

#z score values for anterior and posterior hippocampus so we can visualize how different these values are by income
pingSubset[!is.na(posterior_hpc_plotting), posteriorZ_plot := scale(posterior_hpc_plotting)]
pingSubset[!is.na(anterior_hpc_plotting), headZ_plot := scale(anterior_hpc_plotting)]
...
longpingSubset[region == "posterior_hpc", region2 := 1]
longpingSubset[region == "anterior_hpc", region2 := -1]
...
#full sample
memory_anterior.m = lm(TBX_ibam_scr ~ anterior_hpc + AgeC + GenderEC, data = pingSubset[!is.na(posterior_hpc)]); summary(memory_anterior.m)

memory_anterior.m = lm(TBX_ibam_scr ~ anterior_hpc + AgeC + Age2 + Age3 + GenderEC, data = pingSubset[!is.na(posterior_hpc)]); summaryh(memory_anterior.m)

#low income subsample
memory_anterior_low_income.m = lm(TBX_ibam_scr ~ anterior_hpc + AgeC + GenderEC +logIncome3, data = pingSubset[!is.na(posterior_hpc) & incomeSplit == -1]); summaryh(memory_anterior_low_income.m)

#low income
memory_anterior_low_income.m = lm(TBX_ibam_scr ~ anterior_hpc + AgeC + Age2 +GenderEC, data = pingSubset[!is.na(posterior_hpc) & incomeSplit == -1]); summaryh(memory_anterior_low_income.m)
```

```{r memory ~ posterior hpc}
#full sample
memory_posterior.m = lm(TBX_ibam_scr ~ posterior_hpc  + AgeC + GenderEC , data = pingSubset[!is.na(anterior_hpc)]); summary(memory_posterior.m)

#low income
memory_posterior.m_lowIncome = lm(TBX_ibam_scr ~ posterior_hpc  + AgeC + GenderEC + logIncome3, data = pingSubset[!is.na(anterior_hpc) & incomeSplit == -1]); summaryh(memory_posterior.m_lowIncome)

#high income
memory_posterior.m_highIncome = lm(TBX_ibam_scr ~ posterior_hpc  + AgeC + GenderEC + logIncome3, data = pingSubset[!is.na(anterior_hpc) & incomeSplit == 1]); summary(memory_posterior.m_highIncome)
```

```{r vocab ~ anterior HPC}
#full sample
language_anterior.m = lm(TBX_VOCAB_THETA ~ anterior_hpc + AgeC + GenderEC, data = pingSubset[!is.na(posterior_hpc)]); summary(language_anterior.m)

language_anterior.m = lm(TBX_VOCAB_THETA ~ anterior_hpc + AgeC + Age2 + GenderEC, data = pingSubset[!is.na(posterior_hpc)]); summaryh(language_anterior.m)

#low income
language_anterior_low_income.m = lm(TBX_VOCAB_THETA ~ anterior_hpc + AgeC +  GenderEC, data = pingSubset[!is.na(posterior_hpc) & incomeSplit == -1]); summaryh(language_anterior_low_income.m)

language_anterior_low_income.m = lm(TBX_VOCAB_THETA ~ anterior_hpc + AgeC +  Age2_test + GenderEC, data = pingSubset[!is.na(posterior_hpc) & incomeSplit == -1]); summaryh(language_anterior_low_income.m)
```

```{r vocab ~ posterior HPC}
language_posterior.m = lm(TBX_VOCAB_THETA ~ posterior_hpc + AgeC + GenderEC, data = pingSubset[!is.na(anterior_hpc)]); summaryh(language_posterior.m)

#low income
language_posterior.m = lm(TBX_VOCAB_THETA ~ posterior_hpc + AgeC + GenderEC + logIncome3, data = pingSubset[!is.na(anterior_hpc) & incomeSplit == -1]); summaryh(language_posterior.m)


#high income
language_posterior.m_high = lm(TBX_VOCAB_THETA ~ posterior_hpc + AgeC + GenderEC + logIncome3, data = pingSubset[!is.na(anterior_hpc) & incomeSplit == 1]); summaryh(language_posterior.m_high)
```

```{r processing speed ~ anterior HPC}
processing_speed_anterior.m = lm(TBX_pspac_scr ~ anterior_hpc  + AgeC + GenderEC, data = pingSubset[!is.na(posterior_hpc)]); summaryh(processing_speed_anterior.m)
```

```{r processing speed ~ posterior HPC}
processing_speed_posterior.m = lm(TBX_pspac_scr ~ posterior_hpc + AgeC + GenderEC, data = pingSubset[!is.na(anterior_hpc)]); summaryh(processing_speed_posterior.m)
```
...
memory_anterior_age_interaction.m = lm(TBX_ibam_scr ~ AgeC_test * anterior_hpc + GenderEC, data = pingSubset[!is.na(posterior_hpc)]); summaryh(memory_anterior_age_interaction.m)
```

```{r vocab scores ~ anterior HPC * age}
language_anterior_age_interaction.m = lm(TBX_VOCAB_THETA ~  AgeC * anterior_hpc + logIncome3 + GenderEC, data = pingSubset[!is.na(posterior_hpc)]); summaryh(language_anterior_age_interaction.m)
```

```{r vocab ~ posterior HPC * age}
language_posterior_age_interaction.m = lm(TBX_VOCAB_THETA ~  AgeC * posterior_hpc + GenderEC, data = pingSubset[!is.na(anterior_hpc)]); summaryh(language_posterior_age_interaction.m)

pingSubset$age_categorical <- relevel(as.factor(pingSubset$age_categorical), ref = "8:12")
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc), summaryh(lm(TBX_VOCAB_THETA ~ posterior_hpc  * age_categorical  + AgeC + GenderEC))] #controlling for age

#in discrete age bins
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & age_categorical %in% c("3:7"), summaryh(lm(TBX_VOCAB_THETA ~ posterior_hpc + GenderEC+ AgeC))]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & age_categorical == "8:12", summaryh(lm(TBX_VOCAB_THETA ~ posterior_hpc + GenderEC+ AgeC))]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & age_categorical == "13:17", summaryh(lm(TBX_VOCAB_THETA ~ posterior_hpc+ GenderEC+ AgeC))]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & age_categorical == "18:21", summaryh(lm(TBX_VOCAB_THETA ~ posterior_hpc+ GenderEC+ AgeC))]
```

```{r memory scores ~ posterior HPC * age}
pingSubset[, .(ageround, Age_cat_round)]
pingSubset[roundAge < 8, age_categorical:= "3:7"]
pingSubset[roundAge > 7.99 & roundAge < 13, age_categorical:= "8:12"]
pingSubset[roundAge > 12.99 & roundAge < 18, age_categorical:= "13:17"]
pingSubset[roundAge > 17.99, age_categorical:= "18:21"]

pingSubset[!is.na(anterior_hpc) & !is.na(posterior_hpc) & !is.na(TBX_ibam_scr), .N, by = .(age_categorical)]%>%arrange(age_categorical)

memory_posterior_age_interaction.m = lm(TBX_ibam_scr ~  AgeC * posterior_hpc + GenderEC, data = pingSubset[!is.na(anterior_hpc)]); summaryh(memory_posterior_age_interaction.m)

pingSubset$age_categorical <- relevel(as.factor(pingSubset$age_categorical), ref = "8:12")

pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc), summaryh(lm(TBX_ibam_scr ~ posterior_hpc  * age_categorical + AgeC + GenderEC))]


#in discrete age bins
pingSubset[!is.na(posterior_hpc)&!is.na(anterior_hpc) & age_categorical %in% c("3:7"), summaryh(lm(TBX_ibam_scr ~ posterior_hpc + AgeC + GenderEC))]
Age_catpingSubset[!is.na(posterior_hpc)&!is.na(anterior_hpc) & age_categorical == "8:12", summaryh(lm(TBX_ibam_scr ~ posterior_hpc + AgeC + GenderEC))]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & age_categorical == "13:17", summaryh(lm(TBX_ibam_scr ~ posterior_hpc+ AgeC + GenderEC))]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & age_categorical == "18:21", summaryh(lm(TBX_ibam_scr ~ posterior_hpc + AgeC +GenderEC))]
pingSubset[!is.na(posterior_hpc)&!is.na(anterior_hpc) & !is.na(TBX_ibam_scr), .N, by = age_categorical]
```
...
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & !is.na(TBX_ibam_scr), 
...
toGather <- pingSubset[, .(ID, memory_residualsZ, language_residualsZ, posterior_hpc, anterior_hpc, income3)]
toPlot <- gather(toGather[!is.na(memory_residualsZ) & !is.na(language_residualsZ) & !is.na(anterior_hpc) & !is.na(posterior_hpc)], region, volume, -c(ID, memory_residualsZ, language_residualsZ, income3))

setDT(toPlot)
toPlot[region == "posterior_hpc", region := "Posterior"]
toPlot[region == "anterior_hpc", region := "Anterior"]
...
#toGather <- pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc), .(ID, posterior_hpc, anterior_hpc, logIncome3, AgeC, GenderEC, DeviceSerialNumber2)]
#setDT(longpingSubset)
...
memory_anterior_interaction.m = lm(TBX_ibam_scr ~ anterior_hpc * incomeSplit + AgeC + GenderEC, data = pingSubset[!is.na(posterior_hpc)]); summary(memory_anterior_interaction.m)
```

```{r vocab scores ~ anterior HPC * income subsample}
vocab_anterior_interaction.m = lm(TBX_VOCAB_THETA ~ anterior_hpc * incomeSplit + AgeC + GenderEC, data = pingSubset[!is.na(posterior_hpc)]);summary(vocab_anterior_interaction.m)
```

```{r memory scores ~ posterior HPC * income subsample}
memory_posterior_interaction.m = lm(TBX_ibam_scr ~ posterior_hpc * incomeSplit + AgeC + GenderEC, data = pingSubset[!is.na(anterior_hpc)]); summary(memory_posterior_interaction.m)
```

```{r vocab scores ~ posterior HPC * income subsample}
vocab_posterior_interaction.m = lm(TBX_VOCAB_THETA ~ posterior_hpc * incomeSplit + AgeC + GenderEC, data = pingSubset[!is.na(anterior_hpc)]); summaryh(vocab_posterior_interaction.m)
```
...
memory_anterior_lower_income.m = lm(TBX_ibam_scr ~ anterior_hpc  + AgeC  + GenderEC, data = pingSubset[!is.na(posterior_hpc) & incomeSplit == -1]); summaryh(memory_anterior_lower_income.m)
```

```{r memory scores ~ anterior HPC in higher income subsample}
memory_anterior_higher_income.m = lm(TBX_ibam_scr ~ anterior_hpc  + AgeC + GenderEC, data = pingSubset[!is.na(posterior_hpc) & incomeSplit == 1]); summaryh(memory_anterior_higher_income.m)
```

```{r vocab ~ anterior HPC in lower income subsample}
language_anterior_lower_income.m = lm(TBX_VOCAB_THETA ~ anterior_hpc + AgeC + GenderEC , data = pingSubset[!is.na(posterior_hpc) & incomeSplit == -1]); summary(language_anterior_lower_income.m)
```

```{r vocab ~ anterior HPC in higher income subsample}
language_anterior_higher_income.m = lm(TBX_VOCAB_THETA ~ anterior_hpc + AgeC + GenderEC , data = pingSubset[!is.na(posterior_hpc) & incomeSplit == 1]); summaryh(language_anterior_higher_income.m)
```

Posterior HPC
```{r memory scores ~ posterior HPC in lower income subsample}
memory_posterior_lower_income.m = lm(TBX_ibam_scr ~ posterior_hpc  + AgeC + GenderEC, data = pingSubset[!is.na(posterior_hpc) & incomeSplit == -1]); summary(memory_posterior_lower_income.m)
```

```{r memory scores ~ posterior HPC in higher income subsample}
memory_posterior_higher_income.m = lm(TBX_ibam_scr ~ posterior_hpc  + AgeC + GenderEC, data = pingSubset[!is.na(posterior_hpc) & incomeSplit == 1]); summary(memory_posterior_higher_income.m)
```

```{r vocab ~ posterior HPC in lower income subsample}
language_posterior_lower_income.m = lm(TBX_VOCAB_THETA ~ posterior_hpc + AgeC + GenderEC , data = pingSubset[!is.na(posterior_hpc) & incomeSplit == -1]); summary(language_posterior_lower_income.m)
```

```{r vocab ~ posterior HPC in higher income subsample}
language_posterior_higher_income.m = lm(TBX_VOCAB_THETA ~ posterior_hpc + AgeC + GenderEC , data = pingSubset[!is.na(posterior_hpc) & incomeSplit == 1]); summary(language_posterior_higher_income.m)
```
...
pingSubset[!is.na(anterior_hpc) & !is.na(posterior_hpc) & !is.na(TBX_ibam_scr), TBX_ibam_scrZ := scale(TBX_ibam_scr)]
pingSubset[!is.na(anterior_hpc)& !is.na(posterior_hpc) & !is.na(TBX_VOCAB_THETA), TBX_VOCAB_THETAZ := scale(TBX_VOCAB_THETA)]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)& !is.na(TBX_ibam_scr), anterior_adjustZ := scale(anterior_hpc)]
pingSubset[!is.na(anterior_hpc) & !is.na(posterior_hpc)& !is.na(TBX_ibam_scr), posterior_adjustZ := scale(posterior_hpc)]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)& !is.na(TBX_ibam_scr), whole_adjustZ := scale(whole_adjust)]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)& !is.na(TBX_ibam_scr), AgeZ := scale(AgeC)]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)& !is.na(TBX_ibam_scr), Age2Z := scale(Age2)]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)& !is.na(TBX_ibam_scr), Age3Z := scale(Age3)]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)& !is.na(TBX_ibam_scr), logIncome3Z := scale(logIncome3)]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)& !is.na(TBX_ibam_scr), scanner := DeviceSerialNumber2]
```
...
anterior_memory.y <- lm(TBX_ibam_scrZ ~ anterior_adjustZ + GenderEC + AgeZ + logIncome3Z + scanner, data = pingSubset[!is.na(posterior_hpc)]); summaryh(anterior_memory.y)
anterior_memory.m <- lm(anterior_adjustZ ~logIncome3Z + AgeZ + scanner + GenderEC, data = pingSubset[!is.na(TBX_ibam_scrZ) & !is.na(posterior_hpc)]); summary(anterior_memory.m) #NOTE: is.na(TBX_ibam_scr) is to ensure that only individuals with memory scores are included in mediation model
...
```{r mediation analysis for income- episodic memory score relationship in full sample}
anterior_memory.y_nonlinear <- lm(TBX_ibam_scrZ ~ anterior_adjustZ + GenderEC + AgeZ + Age2Z + Age3Z + logIncome3Z + scanner, data = pingSubset[!is.na(posterior_hpc)]); summaryh(anterior_memory.y)
anterior_memory.m_nonlinear <- lm(anterior_adjustZ ~logIncome3Z + AgeZ + scanner + GenderEC, data = pingSubset[!is.na(TBX_ibam_scrZ) & !is.na(posterior_hpc)]); summary(anterior_memory.m) #NOTE: is.na(TBX_ibam_scr) is to ensure that only individuals with memory scores are included in mediation model
...
anterior_memory.y <- lm(TBX_ibam_scrZ ~ anterior_adjustZ + GenderEC + AgeZ  + logIncome3Z + scanner, data = pingSubset[!is.na(posterior_hpc) & income3 < 76000]); summaryh(anterior_memory.y)
anterior_memory.m <- lm(anterior_adjustZ ~logIncome3Z + AgeZ + scanner + GenderEC, data = pingSubset[!is.na(TBX_ibam_scrZ) & !is.na(posterior_hpc) & income3 < 76000]); summary(anterior_memory.m) #NOTE: is.na(TBX_ibam_scr) is to ensure that only individuals with memory scores are included in mediation model
...
```{r mediation analysis for income- episodic memory score relationship in lower income subsample}
anterior_memory.y_nonlinear <- lm(TBX_ibam_scrZ ~ anterior_adjustZ + GenderEC + AgeZ  + Age2Z + logIncome3Z + scanner, data = pingSubset[!is.na(posterior_hpc) & income3 < 76000]); summaryh(anterior_memory.y_nonlinear)
anterior_memory.m_nonlinear <- lm(anterior_adjustZ ~logIncome3Z + AgeZ + scanner + GenderEC, data = pingSubset[!is.na(TBX_ibam_scrZ) & !is.na(posterior_hpc) & income3 < 76000]); summary(anterior_memory.m_nonlinear) 
...
Education_memory.m <- lm(TBX_ibam_scr ~ educationAvg_recode +AgeC + GenderEC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)]); summaryh(Education_memory.m)
```

```{r vocabulary ~ education models}
Education_language.m <- lm(TBX_VOCAB_THETA ~ educationAvg_recode + GenderEC + AgeC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)]); summaryh(Education_language.m)
```

subregion - education models
```{r anterior vols ~ income}
Education_anterior.m <- lm(anterior_hpc ~ educationAvg_recode + AgeC + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(posterior_hpc)]); summaryh(Education_anterior.m)
```

```{r posterior vols ~ income}
Education_posterior.m <- lm(posterior_hpc ~ educationAvg_recode + AgeC  + GenderEC +  DeviceSerialNumber, data = pingSubset[!is.na(anterior_hpc)]); summaryh(Education_posterior.m)
```
...
Analysis to test whether anterior volumes mediate the relationship between income and memory scores
```{r z score values so we can report standardized coefficients}
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & !is.na(TBX_ibam_scr), educationAvg_recodeZ := scale(educationAvg_recode)]
```

Episodic memory scores - Ahipp volumes  --- Full sample (z-scored mediation) ** only linear effects of age included
```{r mediation analysis for income- episodic memory score relationship in full sample}
source( "http://page-gould.com/scripts/r/mediations_patch.r" ) # Needed to update the mediations function
anterior_memory.y <- lm(TBX_ibam_scrZ ~ anterior_adjustZ + GenderEC + AgeZ + educationAvg_recodeZ + scanner, data = pingSubset[!is.na(posterior_hpc)]); summary(anterior_memory.y)
anterior_memory.m <- lm(anterior_adjustZ ~educationAvg_recodeZ + AgeZ + scanner + GenderEC, data = pingSubset[!is.na(TBX_ibam_scrZ) & !is.na(posterior_hpc)]); summary(anterior_memory.m) #NOTE: is.na(TBX_ibam_scr) is to ensure that only individuals with memory scores are included in mediation model
...
```{r Figure 1c, d}
pingSubset[!is.na(anterior_hpc) & !is.na(posterior_hpc) & !is.na(TBX_ibam_scr), 
           memory_residualsZ := scale(residuals(lm(TBX_ibam_scr ~ AgeC + GenderEC)))]
pingSubset[!is.na(anterior_hpc) & !is.na(posterior_hpc) & !is.na(TBX_VOCAB_THETA), 
...
pingSubset[!is.na(anterior_hpc) & !is.na(posterior_hpc), posterior_hpc_residualsZ := scale(residuals(lm(posterior_hpc ~ AgeC + GenderEC)))]
pingSubset[!is.na(anterior_hpc) & !is.na(posterior_hpc), anterior_hpc_residualsZ := scale(residuals(lm(anterior_hpc ~ AgeC + GenderEC)))]
...
ggplot(longpingSubset_analysis, aes(volume, memory_residualsZ, col = region)) +
   geom_smooth(formula = y ~ x, method = 'lm', col = posteriorCol, data = longpingSubset_analysis[region == "posterior_hpc"]) +
  geom_smooth(formula = y ~ x, method = 'lm', col = anteriorvolscolor, data = longpingSubset_analysis[region == "anterior_hpc"]) +
   labs(y = "Memory (z scores)", x = " ", col = " ")+
     #theme(plot.caption = element_text(hjust = -.2, size  = 12)) +
ggtheme2
...
ggplot(longpingSubset_analysis[income3 < 76000], aes(volume,vocab_residualsZ, col = region)) +
   geom_smooth(formula = y ~ x, method = 'lm', col = posteriorCol, data = longpingSubset_analysis[region == "posterior_hpc" & income3 < 76000]) +
  geom_smooth(formula = y ~ x, method = 'lm', col = anteriorvolscolor, data = longpingSubset_analysis[region == "anterior_hpc"& income3 < 76000]) +
   labs(y = "Vocabulary (z scores)", x = " ", col = " ")+
     #theme(plot.caption = element_text(hjust = -.2, size  = 12)) +
ggtheme2
...
pingSubset[!is.na(anterior_hpc) & !is.na(posterior_hpc), posterior_hpcZ := scale(posterior_hpc)]
pingSubset[!is.na(anterior_hpc) & !is.na(posterior_hpc), anterior_hpcZ := scale(anterior_hpc)]

#age and anterior
ggplot(pingSubset, aes(Age, anterior_hpcZ)) +
...
#age and poserior
ggplot(pingSubset, aes(Age, posterior_hpcZ)) +
...
```{r Suppl Figure 7a}
pingSubset[!is.na(anterior_hpc) & !is.na(posterior_hpc), ]%>%
...
```{r Suppl Figure 8a}
pingSubset[!is.na(anterior_hpc) & !is.na(posterior_hpc), ]%>%
  ggplot(aes(logIncome3, scale(anterior_hpc), col = age_categorical )) +
  stat_smooth(method  = "lm", formula = y~ x) +
ggtheme +
...
```{r Suppl Figure 8b}
pingSubset[!is.na(anterior_hpc) & !is.na(posterior_hpc), ]%>%
  ggplot(aes(logIncome3, scale(posterior_hpc), col = age_categorical )) +
  stat_smooth(method  = "lm", formula = y~ x) +
ggtheme +
...
ggplot(pingSubset, aes(logIncome3, anterior_hpcZ)) +
...
ggplot(pingSubset, aes(logIncome3, posterior_hpcZ)) +
...
ggplot(pingSubset, aes(anterior_hpc, memory_residualsZ)) +
...
ggplot(pingSubset, aes(posterior_hpc, memory_residualsZ)) +
...
ggplot(pingSubset, aes(anterior_hpc, vocab_residualsZ)) +
  geom_jitter(color = anteriorvolscolor, alpha = 0.2) +
  ggtheme +
  stat_smooth(formula = y ~ x, method = "lm", color='black') +
...
ggplot(pingSubset, aes(posterior_hpc, vocab_residualsZ)) +
...
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & !is.na(TBX_ibam_scr), processing_speed_residuals := residuals(lm(TBX_pspac_scr ~ AgeC + GenderEC))]
...
ggplot(pingSubset, aes(anterior_hpc, processing_speed_residuals)) +
  geom_jitter() + 
  ggtheme + 
  stat_smooth(formula = y ~ x, method = "lm")
```
