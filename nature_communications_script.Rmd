...
Income_memory.m <- lm(TBX_ibam_scr ~ logIncome3 +AgeC + GenderEC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc)]); summary(Income_memory.m)
...
Income_memory_income_subsample_low.m_nonlinear <- lm(TBX_ibam_scr ~ logIncome3 + AgeC + Age2 +GenderEC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & incomeSplit == -1]); summaryh(Income_memory_income_subsample_low.m_nonlinear)

#high income
Income_memory_income_subsample_high.m <- lm(TBX_ibam_scr ~ logIncome3 + AgeC  + GenderEC, data = pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & incomeSplit == 1]); summary(Income_memory_income_subsample_high.m)
```
...
pingSubset[, mean(anterior_hpc, na.rm=T)]
pingSubset[, mean(posterior_hpc, na.rm=T)]
...
language_anterior_low_income.m = lm(TBX_VOCAB_THETA ~ anterior_hpc + AgeC +  Age2 + GenderEC, data = pingSubset[!is.na(posterior_hpc) & incomeSplit == -1]); summaryh(language_anterior_low_income.m)
```
...
memory_anterior_age_interaction.m = lm(TBX_ibam_scr ~ AgeC * anterior_hpc + GenderEC, data = pingSubset[!is.na(posterior_hpc)]); summary(memory_anterior_age_interaction.m)
```
...
pingSubset[, round_age := round(Age)]
pingSubset[round_age > 3 & round_age < 8, age_categorical:= "3:7"]
pingSubset[round_age > 7.99 & round_age < 12.99, age_categorical:= "8:12"]
pingSubset[round_age > 12.999 & round_age < 17.99, age_categorical:= "13:17"]
pingSubset[round_age > 17.99, age_categorical:= "18:21"]

pingSubset$age_categorical <- relevel(as.factor(pingSubset$age_categorical), ref = "13:17")
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc), summaryh(lm(TBX_ibam_scr ~ posterior_hpc  * age_categorical + GenderEC))]

#in discrete age bins
pingSubset[!is.na(posterior_hpc)&!is.na(anterior_hpc) & age_categorical %in% c("3:7"), summaryh(lm(TBX_ibam_scr ~ posterior_hpc+ GenderEC))]
pingSubset[!is.na(posterior_hpc)&!is.na(anterior_hpc) & age_categorical == "8:12", summaryh(lm(TBX_ibam_scr ~ posterior_hpc + GenderEC))]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & age_categorical == "13:17", summaryh(lm(TBX_ibam_scr ~ posterior_hpc+ GenderEC))]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & age_categorical == "18:21", summaryh(lm(TBX_ibam_scr ~ posterior_hpc + GenderEC))]
```
...
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc), summaryh(lm(TBX_VOCAB_THETA ~ posterior_hpc  * age_categorical  + GenderEC))] #controlling for age

#in discrete age bins
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & age_categorical %in% c("3:7"), summaryh(lm(TBX_VOCAB_THETA ~ posterior_hpc + GenderEC))]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & age_categorical == "8:12", summaryh(lm(TBX_VOCAB_THETA ~ posterior_hpc + GenderEC))]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & age_categorical == "13:17", summaryh(lm(TBX_VOCAB_THETA ~ posterior_hpc+ GenderEC))]
pingSubset[!is.na(posterior_hpc) & !is.na(anterior_hpc) & age_categorical == "18:21", summaryh(lm(TBX_VOCAB_THETA ~ posterior_hpc+ GenderEC))]
```
